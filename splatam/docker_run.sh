...
docker run -it --volume="/media/raviteja/Disk21/datasets:/root/code/datasets" --volume="/media/raviteja/Disk21/code_outputs/splatam:/root/code/output" --env="NVIDIA_VISIBLE_DEVICES=all" --env="NVIDIA_DRIVER_CAPABILITIES=all" \
    --net=host \
    --ipc=host\
    --privileged \
    --group-add audio \
